...
function list_potition(cIDJbtn){
            ...
            responseGet.map(function(responseGetList){
                ...
                if (status==0) {
                    ...
                    response.map(function(responseList, i){
                        ...
                        var action = '';
                        if (deleted==0) {
                            action += '<button class="btn btn-info" onclick="update(\''+cIDJbtn+'\', \''+cNmJbtn+'\');" title="Update Company - '+cNmJbtn+'."><i class="mdi mdi-lead-pencil"></i></button>&nbsp;&nbsp;';
...
function update(cIDJbtn, cNmJbtn){
    $('#cIDJbtn').val(cIDJbtn);
    $('#cNmJbtn').val(cNmJbtn);
    document.getElementById('cIDJbtn').setAttribute('readonly', 'readonly');
    document.getElementById('cIDJbtn').setAttribute('style', 'color:black');
}

function add_potition(){
    var cIDJbtn = $('#cIDJbtn').val();
    var cNmJbtn = $('#cNmJbtn').val();

    modal_loading_open('bg-info', 'Saving data', 'Please wait...');

    $.ajax({
        url: base_url+'ess/add-potition/'+key_session,
        type: 'post',
        data: 'cIDJbtn='+cIDJbtn+'&cNmJbtn='+cNmJbtn,
        crossDomain: true,
        dataType: 'JSON',
        success: function(responseGet){
            //console.log(responseGet);
...
function reset_form(){
    ...
    $('#cNmJbtn').val('');
    list_potition(0);
    document.getElementById('cIDJbtn').removeAttribute('readonly');
    document.getElementById('cIDJbtn').removeAttribute('style');
    document.getElementById('cIDJbtn').setAttribute('style', 'color:black');
}
