$(document).ready(function(){
    list_salary_component(0);
    kategori_list('');
    trans_manual('');
})

function list_salary_component(cIDKomponen){
    $('#list_salary_component').dataTable().fnDestroy();
    $.ajax({
        url: base_url+'ess/list-salary-component/'+key_session+'/'+cIDKomponen,
        type: 'get',
        crossDomain: true,
        dataType: 'JSON',
        success: function(responseGet){
            //console.log(responseGet);
            $('#list_salary_component tbody').html('');
            responseGet.map(function(responseGetList){
                ...
                if (status==0) {
                    ...
                    response.map(function(responseList, i){
                        ...
                        var action = '';
                        if (deleted==0) {
                            action += '<button class="btn btn-info" onclick="update(\''+cIDKomponen+'\', \''+cNmKomponen+'\', \''+kategori+'\', \''+trans_manual+'\');" title="Update Company - '+cNmKomponen+'."><i class="mdi mdi-lead-pencil"></i></button>&nbsp;&nbsp;';
                            action += '<button class="btn btn-danger" onclick="disable_enable(\''+cIDKomponen+'\', \''+cNmKomponen+'\', \''+1+'\');" title="Disable Company - '+cNmKomponen+'."><i class="mdi mdi-delete"></i></button>';
                        }
                        else {
                            action = '<button class="btn btn-warning" onclick="disable_enable(\''+cIDKomponen+'\', \''+cNmKomponen+'\', \''+0+'\');" title="Enable Company - '+cNmKomponen+'."><i class="mdi mdi-backup-restore"></i></button>';
                        }
                        ...
                        trList += '<tr>';
                            ...
                            trList += '<td style="color:white;">'+trans_manual_descr+'</td>';
                            trList += '<td>'+action+'</td>';
                        trList += '</tr>';
                    })
                    $('#list_salary_component tbody').append(trList);
                    $('#list_salary_component').DataTable( {
                        scrollY:        "600px",
                        scrollX:        true,
                        scrollCollapse: true,
...
function update(cIDKomponen, cNmKomponen, kategori, trans_manual_get){
    //console.log(trans_manual_);
    $('#cIDKomponen').val(cIDKomponen);
    $('#cNmKomponen').val(cNmKomponen);
    kategori_list(kategori);
    trans_manual(trans_manual_get);
    document.getElementById('cIDKomponen').setAttribute('readonly', 'readonly');
    document.getElementById('cIDKomponen').setAttribute('style', 'color:black');
}

function add_salary_component(){
    var cIDKomponen = $('#cIDKomponen').val();
    var cNmKomponen = $('#cNmKomponen').val();
    var kategori = $('#kategori').val();
    var trans_manual = $('#trans_manual').val();

    modal_loading_open('bg-info', 'Saving data', 'Please wait...');

    $.ajax({
        url: base_url+'ess/add-salary-component/'+key_session,
        type: 'post',
        data: 'cIDKomponen='+cIDKomponen+'&cNmKomponen='+cNmKomponen+'&kategori='+kategori+'&trans_manual='+trans_manual,
        crossDomain: true,
        dataType: 'JSON',
        success: function(responseGet){
            //console.log(responseGet);
...
function reset_form(){
    ...
    kategori_list('');
    trans_manual('');
    document.getElementById('cIDKomponen').removeAttribute('readonly');
    document.getElementById('cIDKomponen').removeAttribute('style');
    document.getElementById('cIDKomponen').setAttribute('style', 'color:black');
}

function disable_enable(cIDKomponen_get, cNmKomponen_get, values){
