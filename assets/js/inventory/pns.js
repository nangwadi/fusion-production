...
function list_common_stock(){
    ...
    $('#list_common_stock').DataTable({
        ...
        "order": [],
        "ajax" : {
            'url' : base_url+'inventory/list-inventory-stock-datatable/'+key_session+'/atk_stock/0',
            'type' : 'post'
        },
        "columnDefs": [
            { 
...
function modal_uom_converter(id_inventory_get, inventory_name_get, uom_cd_get, no_get){
    $.ajax({
        url: base_url+'inventory/list-uom-converter/'+key_session+'/pns/'+id_inventory_get,
        type: 'GET',
        crossDomain: true,
        dataType: 'JSON',
        success: function(responseGet){
            ...
            responseGet.map(function(responseGetList){
                ...
                var trList = '';
                if (status==0) {
                    trList += '<option value="">Select UOM</option>';
                    $('#number_convert').val('0');
                }
                else {
                    var response = responseGetList.response;
                    response.map(function(responseList){
                        var id_uom_convert = responseList.id_uom_convert;
                        var uom_cd_convert = responseList.uom_cd_convert;
                        var number_convert = responseList.number_convert;

                        trList += '<option value="'+id_uom_convert+'">'+uom_cd_convert+'</option>';
                        $('#number_convert').val(number_convert);

                    })
                }
                var data_uom = responseGetList.data_uom;
                data_uom.map(function(responseListUom){
                    var id_uom = responseListUom.id_uom;
                    var uom_cd = responseListUom.uom_cd;
                    trList += '<option value="'+id_uom+'">'+uom_cd+'</option>';
                })
                $('#id_uom_convert').append(trList);
            })

            $('#uom_cd_purchase').val('');
            $('#uom_cd_purchase').val(uom_cd_get);
            $('#no_uom').val('');
            $('#no_uom').val(no_get);
            $('#id_inventory_uom').val('');
            $('#id_inventory_uom').val(id_inventory_get);
            $('#inventory_name_uom').val('');
            $('#inventory_name_uom').val(inventory_name_get);
            $('#modal_title_uom_converter').append('Uom Converter of '+inventory_name_get);
            $('#modal_convert').modal('show');
        }
    })
}

function modal_uom_converter_hide(){
    $('#modal_convert').modal('hide');
    list_common_stock();
}

function add_uom_convert(){
    var no_uom = $('#no_uom').val();
    var id_inventory_uom = $('#id_inventory_uom').val();
    var inventory_name_uom = $('#inventory_name_uom').val();
    var id_uom_convert = $('#id_uom_convert').val();
    var number_convert = $('#number_convert').val();

    if (id_uom_convert == '' || number_convert*1 == 0) {
        alert ('UOM Take or Qty Cannot Empty.');
    }
    else {
        modal_uom_converter_hide();
        modal_loading_open('bg-info', 'Saving data', 'Please wait...');
        $.ajax({
            url: base_url+'inventory/add-uom-convert/'+key_session,
            type: 'POST',
            data:'id_inventory='+id_inventory_uom+'&id_uom_convert='+id_uom_convert+'&number_convert='+number_convert,
            crossDomain: true,
            dataType: 'JSON',
            success: function(responseGet){
                console.log(responseGet);
                ...
                responseGet.map(function(responseGetList){
                    ...
                    if (status==1) {
                        ...
                        setTimeout(function () {
                            modal_loading_hide();
                            $('#uom_cd_'+no_uom).html('');
                            $('#uom_cd_'+no_uom).append(response);
                        }, 5000);
                    }
                    else {
                        modal_loading_open('bg-danger', 'Saving data unsuccessfully', response);
                        setTimeout(function () {
                            modal_loading_hide();
                        }, 5000);
                    }
...
function list_annual_price(id_inventory_get){
    ...
    $.ajax({
        url: base_url+'inventory/list-annual-price/'+key_session+'/pns/'+id_inventory_get,
        type: 'GET',
        crossDomain: true,
        dataType: 'JSON',
        success: function(responseGet){
...
function add_annual_price(){
    ...
    if (year == '' || annual_price == '') {
        ...
        $.ajax({
            ...
            type: 'post',
            data: 'year='+year+'&annual_price='+annual_price+'&id_inventory='+id_inventory+'&category=pns',
            crossDomain: true,
            dataType: 'JSON',
            success: function(responseGet){
                //console.log(responseGet);
...
function modal_take(){
    $.ajax({
        url: base_url+'inventory/list-inventory-stock/'+key_session+'/pns/0',
        type: 'GET',
        crossDomain: true,
        dataType: 'JSON',
        success: function(responseGet){
    ...
    if (cNIK_take == '' || cNIK_take == null) {
        ...
        if (resume_length==0) {
            ...
            $.ajax({
                ...
                type: 'post',
                data: 'JobNo='+JobNo+'&date_out='+date_out+'&cNIK_take='+cNIK_take+'&take_list='+take_list_array+'&take_line='+resume_length+'&category=pns',
                crossDomain: true,
                dataType: 'JSON',
                success: function(responseGet){
                    console.log(responseGet);
