...
function list_transaction(){
    ...
    $('#line_description').dataTable({
        ...
        "order": [],
        "ajax" : {
            'url' : base_url+'finance/list-transaction-line-periode-datatable/'+key_session+'/'+transaction_periode,
            'type' : 'post'
        },
        "columnDefs": [
            { 
...
function approve(id_transaction_distribution_get, transaction_number_get, no_get){
    //console.log(id_transaction_distribution_get+' '+transaction_number_get);
    const approve = $('#approve_'+no_get).prop('checked');
    var values = '';
    var note = '';
    if (approve == true) {
        values = 1;
    ...
    if (confirm('Are you sure you want mark as '+note+' '+transaction_number_get+' ?')) {
        modal_loading_open('bg-danger', 'Updating data', 'Please wait...');
        $.ajax({
            url: base_url+'finance/update-checked-transaction-distribution/'+key_session+'/'+id_transaction_distribution_get+'/'+values,
            type: 'get',
            crossDomain: true,
            dataType: 'JSON',
        ...
        if (approve == true) {
            $('#approve_'+no_get).prop('checked', false);
        }
        else {
            $('#approve_'+no_get).prop('checked', true);
        }
    }
}
