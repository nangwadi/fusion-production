$(document).ready(function(){
    ...
    if ((transaction_number_db*1) != 0) {
        detail_header(transaction_number_db);
    }
})
        ...
        success: function(responseGet){
            ...
            responseGet.map(function(responseGetList){
                ...
                if (status==0) {
                    ...
                    var check_po = responseGetList.check_po;
                    ...
                    response.map(function(responseList){
                        ...
                        purchase_order_line_blank(transaction_number_get);
...
function list_inventory_add(category_get){
    ...
    var no_line = $('#no_line_'+category_get).val();
    
    $('#id_inventory_line_'+no_line).val('');
    $('#id_part_list_line_'+no_line).val('');
    $('#inventory_cd_line_'+no_line).val('');
    $('#inventory_name_line_'+no_line).val('');
    $('#JobNo_line_'+no_line).val('');
    $('#qty_order_line_'+no_line).val('');
    $('#qty_receipt_line_'+no_line).val('');
    $('#uom_cd_line_'+no_line).val('');
    $('#unit_price_line_'+no_line).val('');
    $('#line_amount_line_'+no_line).val('');
    $('#discount_amount_line_'+no_line).val('');
    $('#discount_percent_line_'+no_line).val('');
    $('#sub_tax_cd_line_'+no_line).val('');
    $('#item_class_cd_line_'+no_line).val('');
    $('#coa_cd_line_'+no_line).val('');
    $('#coa_name_line_'+no_line).val('');
    $('#warehouse_name_line_'+no_line).val('');
    ...
    if (bom==true) {
        var id_part_list = $('#id_inventory_'+no_select).val();
        ...
        if ((id_part_list_array.includes(id_part_list*1)) == true) {
            ...
            $.ajax({
                url: base_url+'jom/list-part-list-detail/'+key_session+'/'+id_part_list,
                type: 'get',
                crossDomain: true,
                dataType: 'JSON',
                success: function(responseGet){
                    ...
                    responseGet.map(function(responseGetList){
                        ...
                        if (status==0) {
                            ...
                            response.map(function(responseList){
                                ...
                                $('#id_part_list_line_'+no_line).val(id_part_list);
                                $('#id_inventory_line_'+no_line).val(id_inventory);
                                $('#inventory_cd_line_'+no_line).val(inventory_cd);
                                $('#inventory_name_line_'+no_line).val(description);
                                $('#JobNo_line_'+no_line).val(JobNo);
                                $('#qty_order_line_'+no_line).val(qty_order);
...
function purchase_order_line_blank(transaction_number_get){
    console.log(transaction_number_get);
    ...
    $('#line_description').dataTable({
        ...
        "order": [],
        "ajax" : {
            'url' : base_url+'distribution/purchase-order-line-blank/'+key_session+'/'+transaction_number_get,
            'type' : 'post'
        },
        "columnDefs": [
            { 
